...
- common Nova scheduler principes related to scheduler setup, scheduler logic of picking a host and
...
Nova sheduling process is configurable. First of all you can choose what driver for scheduling is
used in *CONF/scheduler/driver* [fn:1]. Several options are available e.g. filter_scheduler,
caching_scheduler or chance_scheduler. The filter_scheduler is used by default and in this doc the
...
1. Get all available compute nodes. Depending on Nova release it either
   could be a request into DB or the Placement service. In Ocata, Placement is in reporting mode
