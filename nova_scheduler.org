...
This doc contains 3 part describing the following:
...
what driver for scheduling is used in *CONF/scheduler/driver*
[1]. Several options are available e.g. filter_scheduler,
caching_scheduler or chance_scheduler. The filter_scheduler is used by
default and in this doc the most attention is focused on it.  Next
important setting is *CONF/scheduler/host_manager* [2] that represents
the in-memory picture of the hosts and their properties that scheduler
takes into account.
...
A good description could be found in filter scheduler Nova docs [3].
...
- availability_zone - a param for create server request [4], restrict list of
